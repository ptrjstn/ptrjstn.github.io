...
export default async function handler(request, response) {
  ...
  const requestedRound = Number.isSafeInteger(Number(request.query?.round)) && Number(request.query.round) >= 0 ? Number(request.query.round) : undefined; const excludeRound = Number.isSafeInteger(Number(request.query?.exclude)) && Number(request.query.exclude) >= 0 ? Number(request.query.exclude) : undefined; const randomRound = request.query?.random === "1" ? (excludeRound === undefined ? Math.floor(Math.random() * puzzles.length) : (excludeRound + 1 + Math.floor(Math.random() * Math.max(1, puzzles.length - 1))) % puzzles.length) : requestedRound; const puzzle = getPuzzle(new Date(), randomRound); if (request.method === "GET") return response.status(200).json({ id: puzzle.id, round: puzzle.round, start: puzzle.start, goal: puzzle.goal, dateLabel: puzzle.dateLabel }); if (request.method !== "POST") return response.status(405).json({ error: "Methode nicht erlaubt." });
