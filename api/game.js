...
export function getPuzzle(now = new Date()) { const date = berlinDate(now); const pair = puzzles[Math.max(0, dayDifference(date)) % puzzles.length]; return { id: `${date}-r${REVISION}`, start: pair[0], goal: pair[1], dateLabel: new Intl.DateTimeFormat("de-DE", { timeZone: "Europe/Berlin", day: "2-digit", month: "2-digit", year: "numeric" }).format(now) }; }
...
async function isNeighbor(current, guess) { const [currentVector, guessVector] = await Promise.all([embedding(current), embedding(guess)]); return similarityToRank(cosineSimilarity(currentVector, guessVector)) <= MAX_NEIGHBOR_RANK; }
...
export default async function handler(request, response) {
  ...
  const puzzle = getPuzzle(); if (request.method === "GET") return response.status(200).json({ id: puzzle.id, start: puzzle.start, goal: puzzle.goal, dateLabel: puzzle.dateLabel }); if (request.method !== "POST") return response.status(405).json({ error: "Methode nicht erlaubt." });
  ...
  try { const dictionary = await dictionaryWord(guess); if (!dictionary || !(await isNeighbor(current || puzzle.start, dictionary))) { await logGameGuess({ puzzle_id: puzzleId, target_word: puzzle.goal, attempt_number: attemptNumber, guess: raw.trim(), normalized_guess: guess, dictionary_word: dictionary, status: "rejected", rank: null, solved: false }); return response.status(422).json({ error: "Nicht dabei" }); } await logGameGuess({ puzzle_id: puzzleId, target_word: puzzle.goal, attempt_number: attemptNumber, guess: raw.trim(), normalized_guess: guess, dictionary_word: dictionary, status: "accepted", rank: null, solved: false }); return response.status(200).json({ word: raw.trim().charAt(0).toLocaleUpperCase("de-DE") + raw.trim().slice(1), solved: false }); } catch { return response.status(502).json({ error: "Die Wortprüfung ist momentan nicht verfügbar." }); }
}
